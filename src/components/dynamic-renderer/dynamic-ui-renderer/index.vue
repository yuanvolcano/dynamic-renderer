...
export default defineComponent({
  ...
  props: {
    ...
    getCurrentComponent: {
      type: Function as unknown as () => (componentName: string) => string,
      required: true,
    },
    dynamicContext: {
      type: Object as () => IDynamicContextReturn,
      required: true,
    },
  },
  setup(props: IProps) {
    ...
    // 获取当前组件类型
    const getCurrentComponentType = (componentName: string) => {
      // 优先使用 componentMap 中的组件
      if (props.componentMap[componentName]) {
        return props.componentMap[componentName];
      }

      // 如果没有找到，使用 getCurrentComponent 方法
      return props.getCurrentComponent(componentName);
    };

    // 渲染子组件
    const renderChildren = (children?: IComponentConfig[]) => {
      if (!children || children.length === 0) {
        return undefined;
      }

      return children.map(child =>
        h(resolveComponent('DynamicUIRenderer'), {
          key: child.id,
          config: child,
          componentMap: props.componentMap,
          getCurrentComponent: props.getCurrentComponent,
          dynamicContext: props.dynamicContext,
        })
      );
    };
    ...
    return () => {
      ...
      // 获取组件类型
      const componentType = getCurrentComponentType(props.config.componentName);

      // 构建组件属性
      const componentProps = {
        ...buildComponentProps(props.config),
        modelValue: modelValue.value,
        style: mergedStyle.value,
        class: mergedClass.value,
        // 事件处理
        onClick: (event: any) => handleClick(event, props.config),
        'onUpdate:modelValue': (event: any) => handleUpdateModelValue(event, props.config),
        onChange: (event: any) => handleEvent('change', event, props.config),
        onFocus: (event: any) => handleEvent('focus', event, props.config),
        onBlur: (event: any) => handleEvent('blur', event, props.config),
      };

      // 渲染组件
      const children = renderChildren(props.config.children);
      return h(componentType, componentProps, children);
    };
  },
});
</script>
